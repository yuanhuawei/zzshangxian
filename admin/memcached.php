...
$this_controller->check_admin_action('memcache') or message('no_privilege');
/*
  +----------------------------------------------------------------------+
  | PHP Version 5                                                        |
  +----------------------------------------------------------------------+
...
if(empty($core->CONFIG['memcache']['server']) || !class_exists('Memcache')){
	exit('memcache!');
}
...
foreach($config['server'] as $v){
	$MEMCACHE_SERVERS[] = $v['host'] .':'. $v['port'];
}
...
list($PHP_SELF) = explode('?', htmlentities(strip_tags($_SERVER['REQUEST_URI'],'')));//isset($_SERVER['PHP_SELF']) ? htmlentities(strip_tags($_SERVER['PHP_SELF'],'')) : '';
...
EOB;
			...
			$sid = array_search($server,$MEMCACHE_SERVERS);
			foreach($entries as $slabId => $slab) {
			    $dumpUrl = $PHP_SELF.'&op=2&server='.$sid.'&dumpslab='.$slabId;
				echo
					"<tr class=tr-$m>",
					"<td class=td-0><center>",'<a href="',$dumpUrl,'#slab_',$sid,'_',$slabId,'" name="slab_',$sid,'_',$slabId,'">',$slabId,'</a>',"</center></td>",
					...
					if ((isset($_GET['dumpslab']) && $_GET['dumpslab']==$slabId) &&  (isset($_GET['server']) && $_GET['server']==array_search($server,$MEMCACHE_SERVERS))){
                        ...
                        foreach($items['ITEM'] as $itemKey=>$itemInfo){
                            ...
                            echo '<a href="',$PHP_SELF,'&op=4&server=',$sid,'&key=',base64_encode($itemKey).'">',$itemKey,'</a>';
                            if ($i++ % 10 == 0) {
                                echo '<br/>';
                            }
