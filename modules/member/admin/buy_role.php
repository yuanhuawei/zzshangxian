...
if(REQUEST_METHOD == 'GET'){
	...
	$select = select();
	$select->from($this_module->TABLE_ .'buy_role');
	...
	include template($this_module, 'buy_role', 'admin');
	
}else if(REQUEST_METHOD == 'POST'){
	
	//修改购买状态,一般用于线下汇款,AJAX调用
	
	$id = isset($_POST['id']) ? filter_int($_POST['id']) : 0;
	$id or exit('[]');
	
	$query = $DB_master->query("SELECT * FROM {$this_module->TABLE_}buy_role WHERE id IN(". implode(',', $id) .") AND status = '0'");
