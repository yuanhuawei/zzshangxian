...
function __construct(&$system, $name){
	...
	$this->last_label_cache = '@'. $this->core->CONFIG['last_last_label_cache'];
...
* 初始化,初始化作用域,角色
**/
function init($SYSTEM, $MODULE, $PAGE, $SITENAME, $ENV){
...
function parse_scope($scope){
	//system-module=site%env#postfix
	$tmp = array();
	$ret = array(
		'system' => '',//-
		'module' => '',//=
	...
	preg_match('/(?<system>\w*)-?(?<module>\w*)=?(?<site>\w*)%?(?<env>\w*)#?(?<postfix>\w*)/', $scope, $tmp);
...
function get_cache(){
	...
	foreach($this->_scopes as $v){
		...
		$label_data = $label_file_read = null;
		$label_file_read = $this->core->CACHE->read('label/#/'. $tmp['system'], $tmp['module'], $tmp['site'].$tmp['env'].$tmp['postfix'] . $this->last_cache, 'serialize');
		if(empty($label_file_read)){
			$label_file_read = $this->core->CACHE->read('label/#/'. $tmp['system'], $tmp['module'], $tmp['site'].$tmp['env'].$tmp['postfix'] . $this->last_label_cache, 'serialize');			
		}
		$label_data = $label_file_read ? $label_file_read : null;
...
function get_data_cache(){
	...
	foreach($this->_scopes as $v){
		...
		$tmp = $this->parse_scope($v);
		$this->_data = array_merge(
			array($v => $this->core->CACHE->read('label/data/'. $tmp['system'], $tmp['module'], $tmp['site'] .$tmp['env'] . $tmp['postfix'] . $this->last_cache, 'serialize')),
			$this->_data
		);
...
function fetch(&$label, $var = null){
	...
	/****** 图片类型的标签 start{*******/
	case 'image':
	if(empty($label['option']['image']['url'])) $label['option']['image']['url'] = "#";
