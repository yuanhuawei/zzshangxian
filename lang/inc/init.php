...
function html_entities($string) {
	...
	if(!is_array($string)) return htmlspecialchars($string, ENT_QUOTES, $charset);
...
function xss_url($url){
	...
	$temp = urlencode($temp);
	$temp = str_replace(array('%3D','%26','%3F','%2F','%3A','%23'),array('=','&','?','/',':','#'),$temp);
...
function init_session(){
	...
	//if(empty($P8SESSION)){
		...
		delete_session('uid = \'0\' AND lastview > '. (P8_TIME - 60) .' AND ip = \''. P8_IP .'\'');
