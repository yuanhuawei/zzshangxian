...
if(REQUEST_METHOD == 'GET'){
	...
	if($role_id){
		$acl = $role->get_acl($this_module, $role_id, $postfix);
	...
	$postfix = isset($_POST['postfix']) ? preg_replace("/[^0-9a-zA-Z_]/", '', $_POST['postfix']) : '';
	
	$role = &$core->load_module('role');
	$controller = &$core->controller($role);
	$controller->set_acl($this_module, $role_id, $acl, $postfix) or message('fail');
