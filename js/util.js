...
(function($) {
    ...
    $.jqDnR = {
        ...
        stop: function(v) { /*E.css('opacity',M.o);*/
            ...
            $().off('mousemove', J.drag).off('mouseup', J.stop);
        }
    };
...
(function($) {
    jQuery.fn.size = function(options) {
        return $(this).length;
    }
})(jQuery)
...
function date(format, timestamp) {
    var that = this,
        jsdate = ((timestamp === undefined) ? new Date() : (typeof(timestamp) === 'object') ? new Date(timestamp) : new Date(timestamp * 1000)),
        formatChr = /\\?([a-z])/gi,
...
function ajax_parameters(o, p) {
    ...
    for (i in o) {
        if (o[i] != null && typeof o[i] == 'object' && !o[i].splice)
...
function ajaxing(options) {
    ...
    if (options.action == 'show') {
        $('#' + options.id).stop(true, true).show().css({
...
function captcha(e, trigger) {
    var c = $('<img src="' + P8CONFIG.url + '/api/captcha.php?_=' + Math.random() + '" align="top" style="cursor: pointer;" onclick="this.src=P8CONFIG.url +\'/api/captcha.php?_=\'+ Math.random()" title="' + P8LANG.refresh_captcha + '" />');
    $('input[type=text][name=captcha]').focus(function() {
        $(e).append(c)
    });
    //$(e).append(c)
}
...
function MoveTabs(id, sid, t, over) {
    ...
    $("#" + id + ">.head >span").off(t).on(t, function() {
...
function P8_Tab(options) {
    ...
    this.add = function(data) {
        ...
        if (this.options.event == 'mouseover') {
            handle.on('mouseover', this._move_event);
        } else {
            handle.on('click', this._move_event);
        }
    ...
    this.remove = function(i) {
        ...
        handle.data('p8_tab_content').off('mouseover', this.mouseover)
            .off('mouseout', this.mouseout)
            .remove();

        handle.removeData('p8_tab_content').removeData('p8_tab_loaded')
            .off('click', this.click)
            .off('mouseover', this.mouseover)
            .off('mouseout', this.mouseout)
            .remove();
...
function Message(option) {
    ...
    this.dialog = new P8_Dialog({
        ...
        ok: function() {
            ...
            $.post(P8CONFIG['URI']['core']['message'].controller + '-message', {
                uid: _this.uid,
                type: type.join(','),
                title: title,
                content: content,
                ...
                if (msg == 'error') {
                    alert(P8LANG.core.notify.error);
                    return;
                }
                ajaxing({
                    action: 'hide',
                    text: P8LANG.core.notify.success
                });
            }, 'json');
        }
    });
    ...
    this.send = function(uid) {
        ...
        $.ajax({
            ...
            success: function(msg) {
                if (msg == 'nouid') {
                    alert(P8LANG.core.notify.nouid);
                    return
                }
...
$(function() {
    ...
    load_css(P8CONFIG.RESOURCE + '/skin/label/jquery-ui.min.css');
