...
function push_item(id, cid){
	...
	$.ajax({
		...
		success: function(json){
			...
			alert(lang_array(P8LANG.office.item.cluster_pushed, [json.length]));
		}
	});
}
...
function collect(id){
	...
	$.ajax({
		url: P8CONFIG['U_controller'] + "/office/item-favory",
		type: 'POST',
		...
		success: function(data){
			if(data == 0){
				ajaxing({text: P8LANG.office.item.collected, action: 'hide'});
			}else if(data == 1){
				ajaxing({text: P8LANG.office.item.collect_success, action: 'hide'});
			}else{
				ajaxing({text: P8LANG.office.item.collect_fail, action: 'hide'});
			}
		}
	});
}
...
function item_operation(ele){
	...
	['<ul style="position: absolute; border: 2px solid #09C; background-color: #ffffff; padding: 5px;">',
		...
		'<li><a router="/'+ SYSTEM +'/item-list?id='+ item_id +'" target="_blank">'+ P8LANG.office.item.more_operation +'</a></li>',
	'</ul>'].join('')
	);
